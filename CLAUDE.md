...
**문서 진실의 출처 (읽는 순서)**: `docs/DOMAIN-RECIPE.md`(도메인 추가·컨벤션·함정) → `AGENTS.md`(전략·방법론, 단 프로토타입 셸 시절 서술 포함). 두 문서가 충돌하면 **DOMAIN-RECIPE.md가 우선**이다 (레포가 인메모리 `app/` 셸 → Prisma + `src/app/` + 쿠키세션 인증으로 진화했고 옛 서술이 일부 남음). **함정**: 두 문서가 참조하는 `docs/KNOWN-GAPS.md`(돈 도메인 하드 원칙·서버측 강제 백로그)는 **현재 레포에 존재하지 않는다** — 참조를 만나면 아래 "횡단 원칙" 요약으로 갈음하고, 그 파일을 찾으려 하지 말 것.
...
티켓 상태 ≠ 수리 상태 · AI 책임 확정 금지(가능성만) · 공백 ≠ 책임 추정(D27) · 결제=완료 후 게이트 · 자동 발송/독촉 금지(명시 승인 후, 1:1 독촉 금지) · 존엄/false-agency 금지. 스코프는 **개인 임대인(집 1~5채·1인 운영)** — 기업 SaaS 스케일 아님.
