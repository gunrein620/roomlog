...
- 기본 검증은 `bash scripts/verify.sh`. API 미기동 상태에서도 web은 데모 폴백으로 렌더돼야 한다.
...
## 2. 실행 / 검증
**개발·테스트 환경의 기준은 docker-compose다.** 호스트에 직접 `pnpm dev`를 띄우지 말고 아래 스택으로 돈다 (web `:3000` · api `:4000` · postgres `:5433`).
```bash
pnpm docker:up                      # = docker compose up --build (스택 기동)
docker compose up -d --build web    # 코드 변경 반영 = 해당 이미지 재빌드 (web/api)
docker compose logs -f web          # 로그
pnpm docker:down                    # 종료
```
검증·테스트:
```bash
bash scripts/verify.sh   # types·ui·web·api 빌드 + api 스모크 (병합/작업 후 기본 검증)
pnpm test:api            # api 유닛 테스트 (DB 연동 테스트는 roomlog-postgres 컨테이너가 떠 있어야 실행, 없으면 skip)
pnpm test:web            # web 유닛 테스트
```
> 호스트에서 개별 프로세스로 띄우는 `pnpm dev:web`(:3000)·`pnpm dev:api`(:4000)도 있지만, 표준 환경은 docker 스택이다.
