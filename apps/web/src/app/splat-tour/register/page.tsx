      ...
      <footer style={footer}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <button
            ...
            style={{
              ...primaryBtn,
              opacity: effectiveTransform ? 1 : 0.45,
              cursor: effectiveTransform ? "pointer" : "not-allowed"
            }}
            disabled={!effectiveTransform}
          ...
          <button
            ...
            disabled={!effectiveTransform || saveState === "saving"}
            onClick={save}
          >
        ...
        <pre style={result}>
          {effectiveTransform
            ? JSON.stringify(effectiveTransform, null, 2)
            : "// 양쪽에서 2점씩 클릭하거나 자동정합 후보를 고르면 SplatTransform이 나타납니다."}
        </pre>
      </footer>
    </div>
  );
