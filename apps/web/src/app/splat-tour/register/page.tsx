...
import type { MitunetSceneLayout, MitunetScenePolygon } from "../../floor-plan-3d/room-scene/mitunet-geometry";
...
import { fetchOwnerListings } from "@/lib/owner-tour-assets";
...
// - 자동(A4): 캡처 도면(RoomPlan, iOS)이 있으면 floor-plan-match 서버 매처가 소유자 도면과 대조해
//   transform을 직접 계산한다. 소스는 두 곳(우선순위, register-capture-source.ts가 판정): 수동
//   "캡처 도면 JSON 업로드"(사용자 행위가 최우선) > 자산에 이미 저장된 SplatAsset.captureFloorPlan
//   (iOS 인테이크가 업로드 때 채운 roomplan.json — 커밋 01ed0afe로 서버 auto-register-preview의
//   기본 소스가 됐다). 시임은 splat-asset-api.ts의 previewAutoRegisterSplatAsset 참조. 캡처 도면이
//   둘 다 없으면 자동 UI는 잠들어 있고(dormant) 기존 수동 흐름이 그대로 동작한다.
...
type PlanSource = "placeholder" | "storage" | "upload" | "listing-db" | "listing-mitunet" | "listing-capture";
...
export default function Page() {
  ...
  // 소스는 두 곳(manual=수동 업로드, asset=SplatAsset.captureFloorPlan) — 우선순위는
  // resolveCaptureFloorPlanSource(수동이 항상 이김, 도착 순서와 무관)가 판정한다.
  const [manualCaptureFloorPlan, setManualCaptureFloorPlan] = useState<RoomPlanCaptureFloorPlan | null>(null);
  const [assetCaptureFloorPlan, setAssetCaptureFloorPlan] = useState<RoomPlanCaptureFloorPlan | null>(null);
  const captureFloorPlanSource = useMemo(
    () => resolveCaptureFloorPlanSource(manualCaptureFloorPlan, assetCaptureFloorPlan),
    [manualCaptureFloorPlan, assetCaptureFloorPlan]
  );
  const captureFloorPlan = captureFloorPlanSource?.plan ?? null;
  ...
  useEffect(() => {
    ...
    // 자산이 연결된 매물의 도면을 픽 화면 도면으로 자동 세팅한다. 우선순위(resolveRegisterPlanDisplay가
    // 판정): 캡처 도면(RoomPlan) > 서버 도면(floorPlanId, 연결만 존중하고 도면은 덮지 않음) >
    // 매물 임베드 벽(walls3D) > mitunet(도면 이미지 자동 추출) > localStorage/placeholder.
    // 캡처는 자산 자체에서 판정 가능(매물 조회 불필요)해 가장 먼저, 그리고 매물 조회 없이 확인한다.
    const applyListingFloorPlan = async (asset: SplatAsset) => {
      const captureDecision = resolveRegisterPlanDisplay(asset, [], undefined);
      if (captureDecision.source === "listing-capture") {
        setPlanShape({ kind: "polygons", layout: captureDecision.layout });
        setPlanSource("listing-capture");
        // 서버 도면 연결(floorPlanId)이 이미 있으면 캡처가 도면 모양은 대체해도 그 연결(가구 매칭용)은 보존한다.
        setPlanServerId(captureDecision.planServerId);
        setPlanPicks([]); // 도면이 바뀌면 기존 도면 픽은 무효
        setPreview(false);
        setPlanMessage(`매물 도면 사용 중 (실측 캡처 도면 · 벽 ${captureDecision.layout.wall.length}개)`);
        return;
      }
      ...
      // walls3D(도면 에디터 저장본)가 있으면 우선 — 실측 박스 벽이라 그대로 쓴다. 없으면
      // mitunet(도면 이미지 업로드→벡터화) 폴리곤으로 폴백한다 — 이미지 업로드 경로는 walls3D를
      // 채우지 않고 mitunet만 남기므로(trade.service.ts normalizeFloorPlan), 여기서 폴백하지
      // 않으면 도면이 있는 매물도 "정합할 도면 없음"으로 보인다. 폴리곤을 박스로 근사하지 않고
      // 그대로 흘린다 — 링 위상(폴리곤+holes) 도면에서 OBB는 방 전체를 덮는 상자 하나로 뭉개졌다
      // (2026-07-2x 걷어냄).
      const editorWalls = planWallsFromPayload({ walls: listing?.floorPlan?.walls3D ?? [] });
      const decision = resolveRegisterPlanDisplay(asset, editorWalls, listing?.floorPlan?.mitunet);

      if (decision.source === "listing-db") {
      ...
      if (decision.source === "listing-mitunet") {
        setPlanShape({ kind: "polygons", layout: decision.layout });
        ...
        setPlanMessage(`매물 도면 사용 중 (도면 이미지에서 자동 추출 · 벽 ${decision.layout.wall.length}개, 정확도 낮음)`);
      }
      // decision.source === "keep" — 매물에 쓸 만한 도면이 없다. localStorage/placeholder를 그대로 둔다.
    };
    ...
    getSplatAsset(queryAssetId)
      .then((asset) => {
        ...
        // 자산에 저장된 캡처 도면(iOS 인테이크가 채운 roomplan.json)이 있으면 자동정합을 깨운다.
        // 무효하면 조용히 무시 — 수동 업로드 경로가 그대로 살아 있다.
        const parsedCapture = parseCaptureFloorPlanJson(asset.captureFloorPlan);
        if (parsedCapture) setAssetCaptureFloorPlan(parsedCapture);
  ...
  // captureMessage는 수동 업로드 시도(성공/실패)에만 채워진다 — 비어 있고 자산에서 자동으로 캡처
  // 도면을 읽었으면 그 사실을 드러내고, 그마저 없으면 기존 "잠들어 있음" 안내로 폴백한다.
  const captureStatusMessage =
    captureMessage ||
    (captureFloorPlanSource?.source === "asset"
      ? `캡처 도면 로드됨 (자산에서 자동 인식 · 벽 ${captureFloorPlanSource.plan.walls.length}개)`
      : "캡처 도면이 없으면 자동정합은 잠들어 있습니다 — 아래 수동 2점 정합을 그대로 쓰세요.");
  ...
  async function uploadCaptureFloorPlanJson(file: File) {
    try {
      ...
      setManualCaptureFloorPlan(plan);
    ...
    <div style={page}>
      ...
      <section style={autoPanel}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <span style={muted}>{captureStatusMessage}</span>
        </div>
            ...
            overlay={
              <div style={planOverlay}>
                ...
                <span style={planSourceChip}>
                  {planSource === "upload"
                    ? planMessage
                    : planSource === "listing-capture"
                      ? `매물 도면 사용 중 (실측 캡처 도면 · 벽 ${planShapeWallCount(planShape)}개)`
...
function PlanPicker({
  ...
  // 좌상단(minX/minZ) — "walls"/"polygons" 두 형태가 공유하는 계산이라 register-plan-source.ts로
  // 뽑아 테스트한다(캡처 도면은 mitunet과 달리 bounds가 원점 대칭이 아니라 역산이 필요 — 그 주석 참조).
  const bounds = planDisplayBounds(plan);
