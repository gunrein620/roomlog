...
export function OverdueWorkspace({ data }: { data: ManagerOverdueWorkspace }) {
  ...
  const [sort, setSort] = useState<OverdueSort>("days_desc");
  const [query, setQuery] = useState("");
  const [selectedId, setSelectedId] = useState<string | undefined>();
  const source = queue === "active" ? data.activeCases : data.waitingCases;
  const visible = useMemo(
    () => sortOverdueCases(filterOverdueCases(source, bucket, query), sort),
    [bucket, query, sort, source],
  );
  ...
  return (
    <>
      ...
      <div className={styles.filterStrip}>
        ...
        <div className={styles.overdueFilterControls}>
          ...
          <label>
            <span className={styles.visuallyHidden}>연체 목록 정렬</span>
            <select
              className={styles.compactSelect}
              value={sort}
              onChange={(event) => setSort(event.target.value as OverdueSort)}
            >
              <option value="days_desc">연체일 높은 순</option>
      ...
      <section className={styles.caseWorkspace}>
        <div className={styles.caseList}>
          ...
          <div className={styles.caseListBody} aria-label="연체 처리 대상 목록">
            {visible.length ? (
              visible.map((item) => (
                <button
                  key={item.billId}
                ...
                >
                  ...
                  <div className={styles.caseMeta}>
                    ...
                    <span className={styles.smallPill}>
                      {queue === "waiting" ? "입금 확인" : overdueStageLabel(item.daysOverdue)}
                    </span>
                  </div>
                </button>
              ))
          ...
          {selected ? (
            <>
              ...
              <div className={styles.detailGrid}>
                ...
                <div className={styles.detailField}><div className={styles.detailLabel}>경과 구간</div><div className={styles.detailValue}>{queue === "active" ? overdueStageLabel(selected.daysOverdue) : "입금 확인 대기"}</div></div>
