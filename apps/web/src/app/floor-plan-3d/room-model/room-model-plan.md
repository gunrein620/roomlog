...
- [ ] `units.ts` 신설: 그리드/축척 상수를 한 곳에 모으고 각 상수의 의미·사용처 주석으로 문서화. 기존 export는 삭제하지 않고 re-export(별칭)로 유지
- [ ] 위 표의 24 vs 25, 1/48 vs 20mm 불일치를 팀에 공유하고 통일안 결정 (제안: 캔버스 기준인 25px·20mm/px로 통일)
- [ ] 합의되면 `summarizeWalls` 계산 + 스펙 기대값 수정 (같은 PR, 리뷰어: 담당 A·C)
