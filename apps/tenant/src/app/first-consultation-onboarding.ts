...
export function firstConsultationOnboarding(room: FirstConsultationRoom) {
  ...
  return {
    ...
    starterPrompts: [
      `${label} 화장실 천장에서 오늘 오전부터 물이 떨어지고 있습니다. 지금도 계속 새고 있고 전기 스위치 근처라 안전 위험이 걱정됩니다. 문제 부위 사진은 아직 없습니다. 오늘 저녁 7시 이후 방문 가능합니다.`,
      `${label} 보일러가 어제 밤부터 작동하지 않아 온수와 난방이 모두 안 됩니다. 가스 냄새는 없지만 안전 확인이 필요합니다. 사진 첨부는 어렵고 보일러 표시창 상태를 글로 설명할 수 있습니다. 내일 오전에 연락 가능합니다.`,
      `${label} 이번 달 관리비 청구 금액이 평소보다 높고 계약서와 다른 것 같습니다. 당장 안전 위험은 없지만 납부 전 확인이 필요합니다. 청구서 사진을 첨부할 수 있습니다. 오늘 오후 6시 이후 확인 가능합니다.`
    ]
  };
}
