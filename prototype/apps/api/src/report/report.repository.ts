...
export abstract class ReportRepository {
  abstract listReports(options?: ReportListOptions): Report[];
  abstract getReport(id: string): Report | undefined;
  abstract listRecipients(): ReportRecipient[];
  abstract listChatMessages(): ChatMessage[];
  abstract listFaqQuestions(): FaqQuestion[];
  abstract getDelivery(reportId: string): ReportDelivery | undefined;
}

const DEMO_RECIPIENTS: ReportRecipient[] = [
  {
    id: "rcp_0001",
    name: "연남 스테이 임대인 (박○○)",
    role: "landlord",
...
const DEMO_REPORTS: Report[] = [
  {
    ...
    disclaimer:
      "AI 정리 스냅샷입니다 — 2026-07-01 09:00 시점 원본 기준. 발송·독촉은 원본 대조 후 진행됩니다.",
    summary:
  ...
  },
  {
    ...
    disclaimer:
      "AI 정리 스냅샷입니다 — 2026-07-02 08:00 시점 원본 기준. 초안 상태이며 임대인 전달 전입니다.",
    summary:
...
const DEMO_CHAT_MESSAGES: ChatMessage[] = [
  ...
  },
  {
    ...
    answer: {
      id: "ans_0001",
      interpretedQuery:
        "연남 스테이 · 2026년 6월 · 미납 세대 조회 (금액성 → 실시간 M-BILL 기준)",
...
const DEMO_DELIVERY: ReportDelivery = {
  reportId: "rpt_0001",
  format: "link",
  masked: true,
  recipient: DEMO_RECIPIENTS[0],
...
export class InMemoryReportRepository implements ReportRepository {
  ...
  private readonly deliveriesByReportId = new Map<string, ReportDelivery>();

  constructor() {
    for (const report of DEMO_REPORTS) {
      this.reports.set(report.id, report);
    }
    this.deliveriesByReportId.set(DEMO_DELIVERY.reportId, DEMO_DELIVERY);
  }

  listReports(options: ReportListOptions = {}): Report[] {
    return Array.from(this.reports.values()).filter(
      (report) =>
        (!options.period || report.period === options.period) &&
        (!options.status || report.status === options.status) &&
        (!options.buildingId || report.scope.buildingId === options.buildingId),
    );
  }
  ...
  getDelivery(reportId: string): ReportDelivery | undefined {
    return this.deliveriesByReportId.get(reportId);
  }
}
