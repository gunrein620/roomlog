...
**상태(2026-07-20)**: HTTP·SSM 디스패치와 **SSM 자가 복구 코드 완성** + API 단위검증
44/44·mesh-worker 스케일 회귀검증 4/4 통과 + **GPU EC2에서 Blender 실물 변환 검증 완료**. 실제
배치는 **GPU EC2 + SSM 디스패치**로 결정했다.
...
확인하지 않았다. **이번 작업에서 추가한 레포 확보→NVMe 재초기화→이미지 재빌드 자가 복구와 SSM
SendCommand→일회성 컨테이너→콜백 경로는 실제 GPU 박스에 배포·실행하지 않았다.** 아래 "배포 전
반드시 확인"의 미체크 항목을 닫기 전엔 프로덕션에 올리지 말 것.
...
시점의 관측값이지 항상 보장되는 SLA가 아니다. 이번에 작성한 `ensure-worker.sh`의 clone/fetch,
`bootstrap-nvme.sh`, 이미지 재빌드, 준비 실패 콜백과 이어지는 CLI 실행은 **실제 박스에서 아직 한 번도
수행하지 않았다.**
...
- [ ] **자가 복구를 실제 GPU 박스에서 검증한다.** stop/start 뒤 레포 확보→NVMe 재초기화→이미지
  재빌드→변환과 준비 실패 콜백을 확인해야 한다. 이번 작업은 `ensure-worker.sh` 작성과 가짜 SSM 명령
  검증까지만 했고 원격에서는 실행하지 않았다.
- [ ] **SSM 경로를 실제 박스에서 끝까지 검증한다.** 현재 리비전으로 `mesh-worker:test`를 다시 빌드한
  뒤 SendCommand→CLI→GLB presigned PUT→성공/실패 콜백을 확인해야 한다. 이번 작업에서는 실제 SSM
  호출이나 원격 컨테이너 실행을 하지 않았다.

### 검증 로그(이 작업 중 실행한 것)

- **자가 복구·SSM 실물 경로 — 미실행.** 이번 작업에서는 실제 AWS `SendCommand`, stop/start,
  GPU 박스의 `ensure-worker.sh`, 이미지 재빌드, 일회성 `mesh-worker:test`, GLB PUT·콜백을 수행하지
  않았다.
- `services/mesh-worker/remote/ensure-worker.sh`: `bash -n` 통과. 실제 root/NVMe/Docker/Git 동작은
  로컬에서 흉내 내지 않았고 원격 실행 검증 대상으로 남겼다.
...
- `services/mesh-worker`: Node 문법 검사와 `node --test services/mesh-worker/*.test.mjs` **8개 전부
  ...
  전송되는 경로를 가짜 입출력으로 확인했다. 실제 Blender 성공 잡과 GPU 박스 컨테이너 실행은 이번
  작업에서 확인하지 않았다.
