...
> 상태: 구현 완료 (2026-07-17) · 작성: 설계 세션 (구현은 서브에이전트/Codex 위임)
>
> **개정 (구현 후)**: 업로드 한도를 이원화 — **직접(S3) 경로 2GB**(`MAX_DIRECT_UPLOAD_BYTES`
> 2000MB, 상한 근거는 sizeBytes int4 범위. 더 키우려면 BigInt 마이그레이션), **멀티파트 폴백은
> 800MB 유지**(힙 버퍼링 경로). 아래 본문의 "800MB" 언급 중 직접 경로에 해당하는 것은 2GB로
> 읽을 것. S3 비활성 환경에서 800MB 초과 파일은 presign 단계에서 즉시 400으로 차단된다.
