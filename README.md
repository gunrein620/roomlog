...
Open the role routes from the single frontend:
...
- Docker Compose 로컬 실행: `NEXT_PUBLIC_API_URL=http://localhost:4000`
- 운영 ALB 경유 실행: `NEXT_PUBLIC_API_URL=/api`
...
1. Open `http://localhost:3000/tenant` and submit the default defect report.
2. Open `http://localhost:3000/manager`, refresh, select the new ticket, and assign the vendor.
3. Open `http://localhost:3000/vendor`, refresh, submit an estimate, save a schedule, and report completion.
4. Return to `http://localhost:3000/manager` and approve completion.
5. Refresh `http://localhost:3000/tenant` to see the completed status.
...
Production routing is expected to be handled by AWS ALB:

- `/`, `/tenant`, `/manager`, `/vendor` -> web service on port 3000
